...
export default function CoursForm({ cancelHandler, onSubmit, IsEdting, defaultvalue }) {
    const [inputs, setInputs] = useState(
        {
            amount: defaultvalue && defaultvalue.amount ? defaultvalue.amount.toString() : '',
            date: defaultvalue && defaultvalue.date ? getFormatedDate(defaultvalue.date) : '',
            description: defaultvalue && defaultvalue.description ? defaultvalue.description : ''
        }
    )
    ...
    function addorUpdateHandler() {
        ...
        const amounIsvalid = !isNaN(courseData.amount) && courseData.amount > 0;
        ...
        if (!amounIsvalid || !dateIsValid || !descriptionIsValid) {
            Alert.alert('Hatalı Giriş', 'Lütfen tüm alanları doldurunuz ve tutar alanına sayısal bir değer giriniz', [{ text: 'Tamam' }])
            return;
        }
        onSubmit(courseData);
    }
